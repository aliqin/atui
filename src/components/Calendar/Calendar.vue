      ...
      getDateRange () {
        ...
        if (firstDayWeek > 1) {
          ...
          for (let i = 1; i < firstDayWeek; i++) {
            ...
            const sclass = this.disabledDate(date) ? `${prefixCls}-calendar-item-disable` : `${prefixCls}-calendar-item-gray`
            this.dateRange.push({
              text: dayText,
              date: date,
              sclass: sclass
        ...
        for (let i = 1; i <= dayCount; i++) {
          ...
          // 今天
          if (i === today.getDate() && today.getFullYear() === time.year && today.getMonth() === time.month) {
            sclass = 'atui-calendar-item-today'
          }
